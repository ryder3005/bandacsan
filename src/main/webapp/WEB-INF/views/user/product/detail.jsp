                    ...
                    <div class="ratio ratio-4x3 mb-3">
                        <c:choose>
                            <c:when test="${not empty product.imageUrls}">
                                <img src="<c:url value='/files/${product.imageUrls[0]}'/>" class="img-fluid rounded" alt="${product.nameVi}" style="object-fit: cover;">
                            </c:when>
                            <c:otherwise>
                    ...
                    <c:if test="${not empty product.imageUrls && product.imageUrls.size() > 1}">
                        <div class="d-flex gap-2 flex-wrap">
                            <c:forEach var="img" items="${product.imageUrls}" begin="0">
                                <img src="<c:url value='/files/${img}'/>" class="rounded border" style="width: 80px; height: 80px; object-fit: cover;" alt="thumb">
                            </c:forEach>
                        </div>
                    </c:if>
                </div>
            ...
            <div class="card shadow-sm">
                <div class="card-body">
                    ...
                    <c:if test="${not empty product.descriptionEn}">
                        <p class="text-muted fst-italic" style="white-space: pre-line;">${product.descriptionEn}</p>
                    </c:if>
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <a href="<c:url value='/user/chat/start/${product.vendorId}'/>" class="btn btn-outline-success">
                                       ...
                                       href="<c:url value='/user/blogs/${blog.id}'/>">
                                        <div>
                                            ...
                                            <small class="text-secondary">
                                                ...
                                                &nbsp;•&nbsp;
                                                <i class="bi bi-calendar"></i> ${blog.createdAt.dayOfMonth}/${blog.createdAt.monthValue}/${blog.createdAt.year}
                                            </small>
                                        </div>
                                    </a>
                                </c:forEach>
