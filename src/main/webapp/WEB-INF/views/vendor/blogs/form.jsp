                ...
                <nav aria-label="breadcrumb" class="animate__animated animate__fadeInUp animate__delay-2s">
                    <ol class="breadcrumb justify-content-center bg-white bg-opacity-20 p-2 rounded-pill">
                        <li class="breadcrumb-item">
                            <a href="<c:url value='/vendor/blogs'/>" class="text-white text-decoration-none">Blog</a>
                        </li>
        ...
        <div class="col-lg-10">
            <div class="card shadow-lg animate__animated animate__fadeInUp">
                ...
                <div class="card-body p-4">
                    <form:form method="post" action="${pageContext.request.contextPath}/vendor/blogs/save"
...
<%--                        </div>--%>
                        ...
                        <div class="mb-4">
                            ...
                            <form:textarea path="contentVi" class="form-control content-editor" rows="20"
                                           placeholder="Viết nội dung bài viết chi tiết bằng tiếng Việt. Bạn có thể sử dụng HTML để định dạng văn bản." />
                            <div class="form-text">
                                <i class="bi bi-info-circle text-muted me-1"></i>
...
<%--                        </div>--%>
                            ...
                            <div class="card bg-light">
                                <div class="card-body">
                                    ...
                                    <div id="productSelection" class="mb-3">
                                        <c:if test="${not empty blog.productIds}">
                                            <c:forEach var="pid" items="${blog.productIds}">
                                                <div class="selected-product">
                                                    <div class="product-info">
                                                        <div class="product-name">Sản phẩm ID #${pid}</div>
                                                        <div class="product-price text-muted">Đã liên kết - giữ nguyên khi lưu</div>
                                                    </div>
                                                    <button type="button" class="btn btn-sm btn-outline-danger"
                                                            onclick="removeProduct('${pid}', this)">
                                                        <i class="bi bi-x"></i>
                                                    </button>
                                        ...
                                        <c:if test="${empty blog.productIds}">
                        ...
                        <div class="d-flex gap-3 mt-5 pt-4 border-top">
                            ...
                            <a href="<c:url value='/vendor/blogs'/>" class="btn btn-outline-secondary btn-lg">
...
<script>
    ...
    let allProducts = [];
    const existingProductIds = [<c:forEach var="pid" items="${blog.productIds}" varStatus="st">${pid}${!st.last ? ',' : ''}</c:forEach>];
    ...
    function loadProducts() {
        ...
        fetch('<c:url value="/vendor/products/search"/>?limit=100')
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                // Auto-select existing items if user already linked products
                if (existingProductIds && existingProductIds.length > 0) {
                    markExistingSelections();
                }
            })
            .catch(error => {
    ...
    function showProductModal(products) {
        ...
        productList.innerHTML = products.map(product => {
            ...
            // Sử dụng cộng chuỗi '+' thay vì dấu backtick hoàn toàn
            let html = '<div class="col-md-6 col-lg-4">';
            html += '<div class="product-item" onclick="toggleProductSelection(' + product.id + ', this)">';
            html += '<div class="form-check mb-2">';
            html += '<input class="form-check-input" type="checkbox" value="' + product.id + '" id="product' + product.id + '">';
            html += '<label class="form-check-label w-100" for="product' + product.id + '">';
            html += '<div class="d-flex align-items-start">';
            html += '<div class="flex-shrink-0 me-2">';
            html += '<div class="product-image-thumbnail">';

            if (hasImage) {
                html += '<img src="/files/' + product.imageUrls[0] + '" alt="' + product.nameVi + '" class="rounded" style="width: 50px; height: 50px; object-fit: cover;">';
            } else {
            ...
            html += '<div class="fw-bold text-truncate">' + product.nameVi + '</div>';
            html += '<div class="text-primary fw-bold">' + product.price.toLocaleString() + '₫f</div>';
            html += '<small class="text-muted">' + (product.categoryNameVi || "Chưa phân loại") + '</small>';
            html += '</div></div></label></div></div></div>';
    ...
    function markExistingSelections() {
        const productSelection = document.getElementById('productSelection');
        if (!productSelection) return;

        // Remove placeholder text
        const placeholder = productSelection.querySelector('.text-center');
        if (placeholder) placeholder.remove();

        existingProductIds.forEach(id => {
            if (!selectedProducts.includes(String(id))) {
                selectedProducts.push(String(id));
                // Try to find product info from loaded list
                const product = allProducts.find(p => p.id == id);
                const productElement = document.createElement('div');
                productElement.className = 'selected-product';
                productElement.innerHTML = `
    <div class="product-info">
        <div class="product-name">\${product != null ? product.nameVi : 'Sản phẩm ID #' + id}</div>
        <div class="product-price">\${product && product.price ? product.price.toLocaleString() + '₫' : ''}</div>
    </div>
    <button type="button" class="btn btn-sm btn-outline-danger" onclick="removeProduct('\${id}', this)">
        <i class="bi bi-x"></i>
    </button>
...
`;
    ...
    // CKEditor 5 setup for content editors
    // CKEditor 5 setup for content editors
    document.addEventListener('DOMContentLoaded', function() {
        const editors = document.querySelectorAll('textarea.content-editor');
        if (editors.length > 0) {
            editors.forEach((textarea) => {
                ClassicEditor.create(textarea, {
                    // CẤU HÌNH UPLOAD ẢNH TẠI ĐÂY
                    ckfinder: {
                        // Trỏ đúng về đường dẫn bạn đã định nghĩa trong FileController
                        uploadUrl: '<c:url value="/files/upload-image"/>'
                    },
                    toolbar: [
                        'heading', '|', 'bold', 'italic', 'link', 'uploadImage', 'blockQuote', // Thêm 'uploadImage'
                        'numberedList', 'bulletedList', '|', 'insertTable', 'undo', 'redo'
                    ]
                }).then(editor => {
                    // Đảm bảo khi submit form, dữ liệu từ CKEditor được đồng bộ vào textarea
                    textarea.form.addEventListener('submit', () => {
                        textarea.value = editor.getData();
                        console.log("data"+editor.getData());
                        if (textarea.value.trim() === '') {
                            showError('Nội dung không được để trống!');
                            submitBtn.disabled = false; // Mở lại nút nếu bị lỗi
                            submitBtn.innerHTML = '<i class="bi bi-check-circle-fill me-2"></i> Xuất bản bài viết';
                        }
                    });
